// FIXME

// import * as config from '$lib/config';
// import { feed } from '$lib/config/general';
// import { favicon } from '$lib/config/icon';
// import { genPosts, genTags } from '$lib/utils/posts';

// const render = (posts = genPosts({ postHtml: true, postLimit: feed.limit, filterUnlisted: true })): string =>
//     `<?xml version='1.0' encoding='utf-8'?>
// <feed xmlns="http://www.w3.org/2005/Atom">
//   <id>${config.protocol + config.domain}/</id>
//   <title><![CDATA[${config.title}]]></title>${config.subtitle ? `\n  <subtitle><![CDATA[${config.subtitle}]]></subtitle>` : ''}${config.favicon ? `\n  <icon>${favicon.src}</icon>` : ''
//         }
//   <link href="${config.protocol + config.domain}" />
//   <link href="${config.protocol + config.domain}/atom.xml" rel="self" type="application/atom+xml" />${feed.hubs?.map(hub => `\n  <link href="${hub}" rel="hub"/>`).join('') ?? ''
//         }
//   <updated>${new Date().toJSON()}</updated>
//   <author>
//     <name><![CDATA[${config.author.name}]]></name>
//   </author>${genTags(posts)
//             .map(tag => `\n  <category term="${tag}" scheme="${config.protocol + config.domain}/?tags=${encodeURI(tag)}" />`)
//             .join('')}${posts
//                 .map(
//                     post => `\n  <entry>
//     <title type="html"><![CDATA[${post.title}]]></title>
//     <link href="${config.protocol + config.domain + post.path}" />
//     <id>${config.protocol + config.domain + post.path}</id>
//     <published>${new Date(post.published ?? post.created).toJSON()}</published>
//     <updated>${new Date(post.updated ?? post.published ?? post.created).toJSON()}</updated>${post.summary ? `\n    <summary type="html"><![CDATA[${post.summary.toString()}]]></summary>` : ''
//                         }
//     <content type="html">
//       <![CDATA[${post.html}]]>
//     </content>${post.tags
//                             ?.map(tag => `\n    <category term="${tag}" scheme="${config.protocol + config.domain}/?tags=${encodeURI(tag)}" />`)
//                             .join('')}
//   </entry>`
//                 )
//                 .join('')}
// </feed>`.trim()

// export const prerender = true
// export const trailingSlash = 'never'
// export const GET = async () =>
//     new Response(render(), {
//         headers: {
//             'content-type': 'application/atom+xml; charset=utf-8'
//         }
//     })

...
export async function load() {
	...
	const parsedActivities = activities.map((activity) => {
		const dates = activity.ActivityDates?.[0]?.ActivityDate?.map((date) => ({
	...
	const filteredActivities = parsedActivities.filter((item) => {
		return item.owner === 'Kunstmuseum Basel' || item.owner === 'Museum der Kulturen Basel' || item.owner === 'Antikenmuseum Basel und Sammlung Ludwig';
	});
